...
if (typeof window !== 'undefined') {
    window.MySpace = window.MySpace || (function() {
        ...
        return {
            ...
            /**
             * ГРАНИЦЫ СУТОК — зеркало серверного `drive_root/db/dates.js`.
             *
             * Дата, выбранная в поле, — это ПОЛНОЧЬ. Поэтому «период по
             * 30 сентября», отправленный на сервер как есть, означает
             * «по 30 сентября 00:00» и выбрасывает весь последний день. Так
             * кассовая книга за месяц теряла обороты 30-го числа целиком.
             *
             * Писать `to + 1 день` нельзя: читается как «зачем-то прибавили
             * сутки», и следующий человек это снимет. Смысл обязан быть назван:
             *
             *     from: MySpace.startOfDay(from).toISOString(),
             *     to:   MySpace.endOfDay(to).toISOString()
             *
             * Сутки МЕСТНЫЕ — «30 сентября» для человека это календарный день
             * там, где он живёт. Пустая дата остаётся пустой: возвращается
             * `null`, растягивать «не заполнено» до конца суток бессмысленно.
             * Правило обязано быть одним на обе стороны, иначе форма и сервер
             * разойдутся ровно на границе суток.
             */
            startOfDay(v) {
                const d = this._asRealDate(v);
                if (!d) return null;
                d.setHours(0, 0, 0, 0);
                return d;
            },
            ...
            /**
             * Конец суток — 23:59:59.999 того же местного дня. Именно так, а не
             * «начало следующего»: граница включающая, и сравнение остаётся
             * `<=`, как его и читают.
             */
            endOfDay(v) {
                const d = this._asRealDate(v);
             ...
             * Пара к `endOfMonth`: период «за сентябрь» задаётся месяцем, а не
             * вычитанием тридцати суток.
             */
            startOfMonth(v) {
                const d = this._asRealDate(v);
                if (!d) return null;
            ...
            /**
             * Конец месяца — последний его день, 23:59:59.999.
             * Написанный на месте вызова `new Date(год, месяц + 1, 0)` даёт
             * ПОЛНОЧЬ последнего дня — именно из-за него кассовая книга теряла
             * обороты 30-го числа целиком.
             */
            endOfMonth(v) {
                const d = this._asRealDate(v);
                if (!d) return null;
                d.setDate(1);                     // 31 января + 1 месяц дало бы март
                d.setMonth(d.getMonth() + 1, 0);  // нулевой день следующего = последний этого
                d.setHours(23, 59, 59, 999);
                return d;
            },

            /** Значение → настоящая дата или `null` (пустая дата — это `null`). */
            _asRealDate(v) {
                if (this.isEmptyDate(v)) return null;
...
class DataForm extends Form {
    ...
    // Важно: новая форма из prefill (напр. счёт «на основании» брони) имеет
    // _modified === false, но _isNew === true — одного _modified недостаточно,
    // иначе серверный вызов уйдёт по UID, которого ещё нет в БД.
    needsSave() {
        return !!(this._modified || this._isNew);
    }
