...
- **Автоматический UID (CRITICAL):** Категорически ЗАПРЕЩЕНО вручную добавлять поле `UID` в определение моделей в `db.json`. Этот реквизит (Primary Key) добавляется централизованно в `events_handler.js` при сборке моделей.
...
- **Обязательные реквизиты доступа:** В проекте действует политика безопасности: каждая таблица (кроме `excluded_tables` в `app.config.json`) ДОЛЖНА содержать хотя бы один реквизит из `required_access_fields` (`organizationId`, `userId`, `hotelId`). Это проверяется при запуске сервера.
- **Фильтрация данных:** Все запросы через `dbGateway` (кроме `admin`) автоматически фильтруются по доступным пользователю организациям, отелям или его собственному ID.
