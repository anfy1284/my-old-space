...
Включает в себя важные механизмы:

1.  **Санитизация данных:** Перед сохранением (create/update) пустые строки `""` преобразуются в `null` для полей с внешними ключами (`references`), чтобы избежать ошибок `SequelizeForeignKeyConstraintError` в PostgreSQL. Обычные текстовые поля `notNull` остаются строками.
2.  **Генерация UID:** Присваивает `UID` новым записям на основе кэша. В качестве ключа хэша всегда используется строго `Model.name` из Sequelize, что гарантирует одинаковый UID для таблицы независимо от того, под каким алиасом она запрашивалась.
