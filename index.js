...
function start(options = {}) {
  ...
  // 1.3: по умолчанию НЕ стартуем — на однопроцессном деплое это лишний Node-процесс
  // (~40-80MB RSS) и HTTP на каждый set/get. Нужен только при кластере/нескольких
  // воркерах → включается через MEMORY_STORE_SERVICE=1.
  if (process.env.MEMORY_STORE_SERVICE === '1') {
    try {
