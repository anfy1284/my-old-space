...
async function ensurePrivateChatsForAllPairs() {
	...
	await sequelize.transaction(async (t) => {
		const users = await modelsDB.Users.findAll({ attributes: ['UID', 'name'], transaction: t });
		...
		for (let i = 0; i < users.length; i++) {
			for (let j = i + 1; j < users.length; j++) {
				...
				for (const members of chatMap.values()) {
					...
					if (set.has(u1.UID) && set.has(u2.UID) && set.size === 2) {
						hasPrivate = true;
						break;
					}
				}
				...
				if (!hasPrivate) {
					try {
						...
						console.error('[messenger:init] Error creating chat for pair', u1.UID, u2.UID, e.message);
					}
				}
			}
		}
...
async function ensureLocalChatIncludesAllUsers() {
	...
	await sequelize.transaction(async (t) => {
		...
		const localChatDefId = '000000000-messenger_chats-0001'; // from apps/messenger/db/defaultValues.json
		...
		const members = await modelsDB.Messenger_ChatMembers.findAll({ where: { chatId: localChat.UID }, transaction: t });
		...
		for (const u of users) {
			if (!existingIds.has(u.UID)) {
				await modelsDB.Messenger_ChatMembers.create({
					...
					userId: u.UID,
					role: u.UID === localChat.userId ? 'owner' : 'member',
