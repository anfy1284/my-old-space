...
function createPayloadStream(opts) {
    ...
    async function* generate() {
        ...
        // Секция пуста намеренно: PK — строковый UID из кода, номера документов
        // считаются как MAX+1. Последовательность `document_moment_seq` (момент
        // времени документа) в копию НЕ кладётся сознательно: восстановить можно и
        // одну организацию, и копию другой инсталляции, поэтому счётчик выводится из
        // самих восстановленных данных — максимум `seq` по документным таблицам
        // (`db/entityMoment.prepare`, зовётся из `restoreFull`). Появится счётчик,
        // который из данных не вывести, — его обязана сохранять эта секция.
