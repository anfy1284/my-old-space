...
async function buildAndLoad(p) {
    ...
    // ── 6. sequences ────────────────────────────────────────────────────────────
    // Секция копии сегодня пуста: первичный ключ строковый и генерится в коде, номера
    // документов считаются как MAX+1. Появится ещё один реальный счётчик — выгрузка
    // обязана его сохранить, а мы здесь — выставить.
    ...
    // Момент времени документов (ТЗ §4.4). Значение последовательности — ЧАСТЬ
    // ДАННЫХ, но в копии его нет и быть не должно: восстановить можно и одну
    // организацию, и копию, снятую другой инсталляцией. Поэтому счётчик выводится
    // из самих данных — максимум `seq` по всем документным таблицам. Без этого
    // шага новые документы получат моменты, уже занятые восстановленными, и
    // порядок учёта сломается МОЛЧА: ни ошибки, ни отказа, просто очередь
    // проведения и остаток «на момент» начнут считать не тот порядок.
    // Теневая схема приезжает вообще без последовательности — создаём её здесь,
    // а не полагаемся на последующую штатную миграцию: база обязана быть целой
    // уже в момент переключения.
    try {
        const entityMoment = require('../db/entityMoment');
        const tables = entityMoment.documentTables(dumpModels);
        if (tables.length) {
            const r = await entityMoment.prepare(sequelize, { schema, tables });
            report('sequences', `Момент времени документов: счётчик на ${r.sequence}`
                + (r.filled ? `, присвоен ${r.filled} строкам` : ''));
        }
    } catch (e) {
        log.warn(`[restoreFull] Момент времени документов не выставлен: ${e.message}`);
    }
