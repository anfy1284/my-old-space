...
const DEFAULTS = {
    ...
    // Внешние приложения-хранилища: вход по подписи Ed25519 (§5, `apiAuth.js`).
    // Здесь лежат только ПУБЛИЧНЫЕ ключи — секрета в этом файле нет.
    apiClients: [],

    // Не прореживать копии, которых хранилище ещё не подтвердило (`ack`).
    // Выключено по умолчанию: включать имеет смысл, только когда хранилище УЖЕ
    // работает, иначе сервер копит файлы, ожидая подтверждения от того, кого нет.
    pruneOnlyAcked: false,
    // Предохранитель к предыдущему: молчащее хранилище не должно забить диск. По
    // истечении срока неподтверждённая копия прореживается на общих основаниях.
    keepUnackedMaxDays: 30
};
