 ...
 *
 * CSS вынесен из «кэшируем на сутки» в «всегда ревалидировать». Причина: таблица
 * стилей это КОД приложения, он меняется с каждой правкой фреймворка, а URL у него
 * постоянный. С `max-age=86400` браузер сутки не спрашивал сервер вовсе — и любая
 * правка вида не доходила до пользователя, пока он не сделает жёсткое обновление.
 * Поймано владельцем: «я по-прежнему вижу кнопки календаря в таблице», хотя правка
 * была на месте. Иконки и шрифты остаются с длинным сроком: они меняются редко и
 * их десятки на форму — ревалидировать каждую значит вернуть шквал запросов, ради
 * которого этот кэш и вводился.
 *
 * Ревалидация дешёвая: ETag/Last-Modified на месте, неизменённый файл отдаётся как
 * 304 без тела.
 */
function fileHeaders(stat, contentType, maxAge) {
    const age = maxAge == null ? 86400 : maxAge;
    const isStylesheet = /^text\/css\b/.test(String(contentType || ''));
    return {
        'Content-Type': contentType,
        'Cache-Control': isStylesheet ? 'no-cache' : `public, max-age=${age}`,
        'ETag': fileETag(stat),
